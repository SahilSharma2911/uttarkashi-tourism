...
import { Phone, Mail, Check, ChevronDown, ChevronRight, Menu, X } from "lucide-react";
...
const Navbar = () => {
  ...
  const toggleMobileSubDropdown = (index) => {
    setActiveMobileSubDropdown(activeMobileSubDropdown === index ? null : index);
  };
        ...
        <div className="py-2 md:py-3 w-11/12 mx-auto flex items-center justify-center">
          <div className="flex flex-wrap items-center justify-center gap-2 md:gap-8">
            ...
            <div className="flex items-center gap-1 md:gap-2">
              ...
              <span className="text-[#e1ed00] text-xs md:text-sm">Contact us</span>
            </div>
            <div className="flex items-center gap-1 md:gap-2">
              <Check className="w-3 h-3 md:w-4 md:h-4 text-white" />
              <span className="text-[#e1ed00] text-xs md:text-sm">About us</span>
            </div>
          </div>
        </div>
      </section>
                  ...
                  <div className="pl-4 pb-2">
                    {item.subItems.map((subItem, subIndex) => (
                      <div key={subIndex} className="py-1">
                            ...
                            >
                              <ChevronDown 
                                className={`w-3 h-3 transition-transform duration-200 ${
                                  activeMobileSubDropdown === subIndex ? 'rotate-180' : ''
                                }`} 
                              />
                            </button>
                          )}
                        ...
                        {subItem.hasSubDropdown && activeMobileSubDropdown === subIndex && (
                          <div className="pl-4">
